...
float   sGetCurrentTime( )
{
	...
	return (startCounters * 1.0f) / contersPersecond;
...
void CreateTestCase( )
{
	...
	cg1->AddTestCase("abceababcabce", false);
	...
	cg2->AddTestCase("abcdabcdbcdcd", false);
	...
	cg5->AddTestCase("ghefvghgh", false);
	cg5->AddTestCase("cdefvcdcd", false);
	...
	cg6->AddTestCase("abefgnwvmvmtuabgnwvmvmvmgabefgnwvmvmvmend", false);
	cg6->AddTestCase("abababefgnwvmvmtuabgnwvmvmvmgabababefgnwvmvmvmend", false);
	cg6->AddTestCase("abababefgnwvmvmtuabgnwvmvmvmgababefgnwvmvmvmend", false);
	cg6->AddTestCase("efgnwtugnwgefgnwend", false);
	...
	cg7->AddTestCase("abnmefvnmabnm", true); //\\0 match nm, \\1 match abnm.
	...
	cg8->AddTestCase("abcdghefgh", false);
	cases.push_back(cg8);
	...
	c18_2->AddTestCase("ababdefnababdeab", true);
	...
	c18_3->AddTestCase("abcdcdefgnabcdcdefcd", true);
	...
	c18_4->AddTestCase("abcdcdgnabcdcdcd", true);
	c18_4->AddTestCase("abcdcdgncdcdabcdcd", false);
	c18_4->AddTestCase("abcdgnabcdcd", true);
	...
	c18_7->AddTestCase("abefbcdefgmcdbcdef", false);
	c18_7->AddTestCase("abefbcdefgmbcdefcd", true);
	...
	for (size_t i = 0; i < cases.size(); ++i)
	{
		...
		it != cases[i]->txt2match_.end(); ++it)
		{
				...
				bool ret = cases[i]->regex.Match((char*)it->first.c_str());
				if (ret != it->second)
				{
					printf("Case: %s, %s, %d\n", cases[i]->pattern_.c_str(), it->first.c_str(), it->second);
				}
...
void TestRegex()
{
	char* hrefPatter = "<\\s*a\\s+href\\s*=\\s*([\"'][^\"']+[\"']|[^\\s>]+)(\\s+[\\w-]+=\\s*([\"'][^\"']+[\"']|[^\\s>]+))*\\s*>(.*?)<\\s*/\\s*a\\s*>";
	//char* expr = "\\w+(\\d{4}-\\d{1,2}-\\d{1,2}),";
	char* imgPatter = "<\\s*img\\s+([\\w-]+=\\s*\"([^\"]+)\"\\s+)*?src\\s*=\\s*([^\"'\\s]+|[\"']([^\"']+)[\"'])(\\s*[\\w-]+\\s*=\\s*\"([^\"]+)\")*\\s*/?>";

	//char* imgPatter = "<\\s*img\\s+(\\w+=\\s*\"[^\"]+\"\\s+)?src";

	//bool b = Match("([1-5w]{2,6}1[34578][0-9]\\d{8}[a-g]{1,5}|abc{2,5}){2}ef", "abcc1113564482596ddef");
	//bool b = Match("([1-5w]{2,6}1[34578][0-9]\\d{8}[a-g]{1,5}|abc{2,5}){2}ef", "abccccc11113564482596ddddef");
	//bool b = Match("a+[a-z]{1,3}\\d{1,6}[a-z]{1,3}_", "aaaaaa123453ddd_");
	//	Regex testregex(imgPatter);
	//	bool ret = testregex.Match("<img id=\"sogouLogo\" alt=\"หันท\" class=\"sosoLogo\" src=\"http://mat1.gtimg.com/www/images/qq2012/sogouSearchLogo20140629.png\" style=\"width:23px;height:23px;\" />");


	//CreateTestCase();
	Regex regex(imgPatter);
	...
	std::string kHtmlSnippet = buffer;
	try
	{
		i = 0;
		f = sGetCurrentTime();
		std::regex img_regex(imgPatter);
		while (i++ < 100)
		{
